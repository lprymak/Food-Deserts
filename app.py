...
from sqlalchemy import create_engine, func, desc, inspect, text
...
results = pd.read_sql(text("select * from full_set_bus where Population_15 > 20000 and LAPOP1_10_y15 != 0"), con=engine.connect())
stateresults = pd.read_sql(text("""SELECT State, SUM(LAPOP1_10_y15) as low_access, SUM(Population_15) as pop_15, SUM(Population_14) as pop_14, SUM(Population_13) as pop_13, SUM(Population_12) as pop_12, SUM(Population_11) as pop_11, SUM(Population_10) as pop_10,
    ...
    FROM full_set_bus GROUP BY State;"""), con=engine.connect())
# print(results)
